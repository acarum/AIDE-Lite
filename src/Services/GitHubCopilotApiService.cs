...
// GitHub Copilot API integration — streaming with tool-use support
// Uses existing GitHub Copilot license (no additional API costs)
// ============================================================================
...
public class GitHubCopilotApiService : IAIApiService
{
    ...
    // GitHub Copilot uses Azure OpenAI endpoint with GitHub token authentication
    private const string ApiUrl = "https://api.github.com/copilot/completions";
    ...
    public GitHubCopilotApiService(
    ...
    /// <summary>
    /// Send a streaming request to the GitHub Copilot API with tool definitions.
    /// Retries automatically on rate-limit (429), overloaded (529), and transient 5xx errors.
    /// Uses GitHub Personal Access Token (same as used for VS Code Copilot).
    /// </summary>
        ...
        CancellationToken externalToken = default)
    {
        ...
        if (string.IsNullOrEmpty(apiKey))
            return ApiResponse.Error("No GitHub token configured. Please set your GitHub Personal Access Token in Settings.");
        ...
        try
        {
            ...
            _logService.Info($"AIDE Lite: [GitHub Copilot API] Model: {config.SelectedModel}, MaxTokens: {config.MaxTokens}, body: {requestJson.Length} chars");
            _logService.Info($"AIDE Lite: [GitHub Copilot API] System prompt sizes: instructions={systemPrompt.StaticInstructions.Length}chars, context={systemPrompt.AppContext.Length}chars");
    ...
    {
        // Convert system prompt to GitHub Copilot format (OpenAI-compatible)
        ...
        var body = new Dictionary<string, object>
        {
            ...
            ["temperature"] = 0.3 // GitHub Copilot works well with lower temperature for code
        };

        if (tools is { Count: > 0 })
        {
            // Convert tools to OpenAI format (GitHub Copilot uses same format)
            var copilotTools = tools.Select(tool => new Dictionary<string, object>
            {
                ["type"] = "function",
                ["function"] = tool
            }).ToList();
            body["tools"] = copilotTools;
        }
        ...
        Action<int, int, int>? onRetryWait, CancellationToken ct)
    {
        ...
        _logService.Info($"AIDE Lite: [GitHub Copilot API] Retry config: max={maxRetries}, delay={retryDelay}s");

        for (var attempt = 0; attempt <= maxRetries; attempt++)
        {
            ct.ThrowIfCancellationRequested();

            _logService.Info($"AIDE Lite: [GitHub Copilot API] Attempt {attempt + 1}/{maxRetries + 1}...");
            ...
            _logService.Info($"AIDE Lite: [GitHub Copilot API] HTTP {statusCode} {response.StatusCode}");

            if (response.IsSuccessStatusCode)
            {
                var result = await ParseStream(response, onTextDelta, onToolStart, ct);
                _logService.Info($"AIDE Lite: [GitHub Copilot API] Tokens - input={result.InputTokens}, output={result.OutputTokens}");
                return result;
            }

            var errorBody = await response.Content.ReadAsStringAsync(ct);
            var truncatedBody = errorBody.Length > 500 ? errorBody[..500] + "...(truncated)" : errorBody;
            _logService.Error($"AIDE Lite: [GitHub Copilot API] Error body: {truncatedBody}");

            if (statusCode == 401 || statusCode == 403)
                return ApiResponse.Error("Invalid or expired GitHub token. Please check your settings and ensure you have an active Copilot license.", "auth_error");
            ...
            _logService.Info($"AIDE Lite: [GitHub Copilot API] Retryable {statusCode}, waiting {delaySec}s...");
    ...
    private static HttpRequestMessage CreateHttpRequest(string apiKey, string json)
    {
        ...
        // GitHub Copilot uses Bearer token authentication (GitHub PAT)
        request.Headers.Add("Authorization", $"Bearer {apiKey}");
        //request.Headers.Add("Editor-Version", "aide-lite/1.0");
        //request.Headers.Add("Editor-Plugin-Version", "aide-lite/1.0");
    ...
    private static ApiResponse FinalErrorForStatus(int statusCode, int maxRetries)
    {
        if (statusCode is 429 or 529)
            return ApiResponse.Error(
                $"Rate limit exceeded after {maxRetries} retries. GitHub Copilot has usage limits. Please try again later.",
                "rate_limit");
    ...
    // ?? SSE stream parsing ??????????????????????????????????????????????
    //
    // GitHub Copilot streams Server-Sent Events (SSE) in OpenAI-compatible format
